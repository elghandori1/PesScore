...
function NewMatch() {
  ...
  useEffect(() => {
    const fetchFriend = async () => {
      try {
        const res = await axios.get(`http://localhost:5000/friends/list-friends`, {
  ...
  return (
    ...
    <main className="relative z-10 flex flex-col items-center min-h-screen pt-20 pb-28 px-1 sm:px-4">
      <section className="bg-white/90 backdrop-blur-sm p-3 sm:p-8 rounded-xl shadow-xl w-full max-w-xs sm:max-w-md mx-auto">
        ...
        {error && (
          <div className="bg-red-50 text-red-600 p-2 sm:p-3 rounded-md mb-3 sm:mb-4 text-xs sm:text-sm">
            {error}
          </div>
        )}
        {success && (
          <div className="bg-green-50 text-green-600 p-2 sm:p-3 rounded-md mb-3 sm:mb-4 text-xs sm:text-sm">
            {success}
          </div>
        )}
        ...
        <form onSubmit={handleSubmit} className="space-y-4 sm:space-y-5">
          <div className="text-center mb-4 sm:mb-6">
            <h2 className="text-xl sm:text-2xl font-bold text-blue-600 mb-1 sm:mb-2">New Match</h2>
            <p className="text-gray-600 text-xs sm:text-sm">Record your football match with {friend?.name_account || 'your friend'}</p>
          </div>
          ...
          <div className="flex flex-col items-center mb-3 sm:mb-4">
            <div className="text-center">
              <p className="text-base sm:text-lg font-medium text-blue-600">
                {friend?.name_account || 'Unknown'}
              </p>
              <p className="text-xs sm:text-sm text-gray-500">ID: {friend?.id_account || ''}</p>
            </div>
          </div>
            ...
            <div className="w-full sm:w-1/2">
              ...
              <div className="relative">
                <input
                  ...
                  className="w-full px-3 py-2 sm:px-4 sm:py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 text-base sm:text-lg text-center"
                  required
                  disabled={loading}
                />
                <div className="absolute inset-y-0 right-0 flex items-center pr-2 sm:pr-3 pointer-events-none">
                  <span className="text-gray-500 text-xs sm:text-sm">goals</span>
                </div>
              </div>
            </div>
          ...
          <button
            type="submit"
            className="w-full bg-gradient-to-r from-blue-600 to-blue-500 hover:from-blue-700 hover:to-blue-600 text-white font-semibold py-2 sm:py-3 px-3 sm:px-4 rounded-lg shadow-md hover:shadow-lg transition-all duration-200 flex items-center justify-center text-base sm:text-lg"
            disabled={loading}
          >
            {loading ? (
              <>
    ...
    {/* Footer */}
    <footer className="fixed bottom-0 w-full bg-gradient-to-r from-blue-800 to-blue-700 text-white text-center py-2 sm:py-4 z-10 backdrop-blur-sm">
