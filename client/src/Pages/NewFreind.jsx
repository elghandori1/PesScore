...
function NewFriend() {
  ...
  return (
    ...
    <header className="w-full flex flex-row justify-between items-center px-2 py-2 sm:px-4 sm:py-3 text-white fixed z-30 bg-black/70 backdrop-blur-sm">
      ...
      <Link 
        to="/"
        className="bg-white/20 hover:bg-white/40 text-white font-semibold px-3 py-1.5 sm:px-4 sm:py-2 rounded-full transition text-xs sm:text-sm md:text-base flex items-center gap-1"
      >
    ...
    <main className="relative z-10 flex flex-col items-center min-h-screen pt-20 pb-24 px-2 sm:px-4">
      <section className="bg-white/90 backdrop-blur-sm p-3 sm:p-6 md:p-8 rounded-xl shadow-xl w-full max-w-md sm:max-w-xl mx-auto">
        <div className="text-center mb-4 sm:mb-6">
          <h2 className="text-lg sm:text-2xl font-bold text-blue-700 mb-1">Add Friend</h2>
          <p className="text-gray-600 text-xs sm:text-sm">Connect with other football enthusiasts</p>
        </div>
        ...
        {/* Search Section */}
        <div className="flex sm:items-center gap-2 mb-3 w-full">
          <input
            type="text"
            placeholder="Search by name or game ID"
            value={search}
            onChange={(e) => setSearch(e.target.value)}
            className="w-full p-3 border border-gray-300 rounded-full px-4 sm:px-5 text-xs sm:text-base focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition"
            disabled={loading}
        ...
        {hasSearched && (
          <div className="mb-4">
            <h3 className="text-base sm:text-lg font-semibold text-blue-600 mb-2">Search Results</h3>
            {users.length > 0 ? (
              <div className="space-y-2 sm:space-y-3">
                {users.map((user) => (
                  <div key={user.id} className="flex flex-row justify-between items-start sm:items-center bg-white p-3 sm:p-4 rounded-lg shadow-sm border border-gray-100">
                    <div>
                      <p className="font-medium text-sm sm:text-base">{user.name_account}</p>
                      <p className="text-xs sm:text-sm text-gray-500">ID: {user.id_account}</p>
                    </div>
                    <button
                      onClick={(e) => handleSendFriendRequest(e, user.id)}
                      className="mt-2 sm:mt-0 bg-gradient-to-r from-blue-500 to-blue-600 text-white px-3 py-1.5 sm:px-4 sm:py-2 rounded-full text-xs sm:text-sm font-medium hover:from-blue-600 hover:to-blue-700 transition-all duration-200 shadow-sm flex items-center gap-1"
                      disabled={loading}
                    >
                      <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4" viewBox="0 0 20 20" fill="currentColor">
              ...
              <div className="text-center py-3 sm:py-4 text-gray-500 text-xs sm:text-base">
        ...
        <div className="bg-gray-50 p-3 sm:p-5 rounded-xl border border-gray-200 shadow-sm mt-2">
          <h3 className="text-base sm:text-lg font-semibold text-gray-800 mb-2 sm:mb-3 flex items-center gap-2">
            ...
            <span className="text-blue-600">Pending Friend Requests ({pendingRequests.length})</span>
          </h3>
            ...
            <div className="max-h-[330px] overflow-y-auto space-y-2">
              {pendingRequests.map((request) => (
                <div key={request.id} className="flex justify-between items-start sm:items-center bg-red-100 p-3 sm:p-4 rounded-lg">
                  <div>
                    <p className="font-medium text-sm sm:text-base">{request.name_account}</p>
                    <p className="text-xs sm:text-sm text-gray-600">Sent: {new Date(request.created_at).toLocaleDateString()}</p>
                  </div>
                  <button
                    onClick={(e) => handleCancelFriendRequest(e, request.id)}
                    className="mt-2 sm:mt-0 bg-gradient-to-r from-red-500 to-red-600 text-white px-3 py-1.5 sm:px-4 sm:py-2 rounded-full text-xs sm:text-sm font-medium hover:from-red-600 hover:to-red-700 transition-all duration-200 shadow-sm flex items-center gap-1"
                    disabled={loading}
                  >
                    <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4" viewBox="0 0 20 20" fill="currentColor">
    ...
    {/* Footer */}
    <footer className="fixed bottom-0 w-full bg-gradient-to-r from-blue-800 to-blue-700 text-white text-center py-4 sm:py-4 z-10 backdrop-blur-sm">
