  ...
  <main className="relative z-10 flex flex-col items-center min-h-screen pt-16 pb-24 px-2 sm:px-4">
    <section className="bg-white/90 backdrop-blur-sm p-3 sm:p-6 rounded-xl shadow-xl w-full max-w-lg sm:max-w-2xl mx-auto">
      <div className="text-center mb-4 sm:mb-6">
        <h2 className="text-xl sm:text-2xl font-bold text-blue-700 mb-1 sm:mb-2 font-almarai">إدارة الأصدقاء</h2>
        <p className="text-gray-600 text-xs sm:text-sm font-almarai">تواصل مع أصدقائك في كرة القدم وإدارتهم</p>
      </div>
      ...
      {success && (
        <div className="bg-green-50 text-green-600 text-center p-3 rounded-md mb-4 text-sm font-almarai">
          {success}
        </div>
      )}
      ...
      <div className="flex flex-row flex-wrap justify-around border-b border-gray-200 mb-4 sm:mb-5">
        ...
        >
          ...
          الأصدقاء
          {activeTab === "friends" && (
            <span className="bg-blue-100 text-blue-800 text-xs px-2 py-0.5 rounded-full font-almarai">
      ...
      {activeTab === "friends" && (
        <div className="relative mb-3 sm:mb-5">
          ...
          <input
            type="text"
            placeholder="البحت عن صديق..."
            value={searchQuery}
            onChange={(e) => setSearchQuery(e.target.value)}
            className="block w-full pl-10 pr-3 py-2 border border-gray-300 rounded-lg bg-white shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 text-xs sm:text-base font-almarai"
          />
        </div>
      )}
        ...
        <div className="max-h-[330px] overflow-y-auto space-y-2">
          {filteredFriends.length === 0 ? (
            <div className="text-center py-6 sm:py-8">
              ...
              <h3 className="mt-2 text-base sm:text-lg font-medium text-gray-900 font-almarai">
                {searchQuery ? "لا يوجد أصدقاء مطابقون لبحثك" : "لا يوجد أصدقاء بعد"}
              </h3>
              <p className="mt-1 text-xs sm:text-sm text-gray-500 font-almarai">
                {searchQuery ? "جرب مصطلح بحث مختلف" : "أضف أصدقاء لبدء تتبع المباريات معًا"}
              </p>
            </div>
          ) : (
              ...
              >
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-xs sm:text-sm font-medium text-gray-900 truncate font-almarai">
                      <span>{friend.name_account}</span>
                      {((pendingMatches[friend.id] || 0) > 0 || (deletionRequests[friend.id] || 0) > 0) && (
                        <span className="ml-2 inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium bg-red-100 text-red-800 font-almarai">
                          {(pendingMatches[friend.id] || 0) + (deletionRequests[friend.id] || 0)} معلقة
                        </span>
                      )}
                    </p>
                    ...
                    {friend.status === "pending_removal" && (
                      <p className="text-[10px] sm:text-xs mt-1 text-red-600 font-almarai">
                        {friend.requested_by === userId
                          ? "لقد طلبت إزالة هذا الصديق"
                          : "هذا الصديق يريد إزالتك"}
                      </p>
                    )}
                  </div>
                </Link>
                  ...
                  {friend.status === "pending_removal" ? (
                    friend.requested_by === userId ? (
                      <button
                        ...
                        className="inline-flex items-center px-2.5 py-1 border border-transparent text-xs font-medium rounded-full shadow-sm text-white bg-gray-600 hover:bg-gray-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-gray-500 font-almarai"
                      >
                        إلغاء
                      </button>
                    ) : (
                      <div className="flex flex-row space-x-2">
                        <button
                          ...
                          className="inline-flex items-center px-2.5 py-1 border border-transparent text-xs font-medium rounded-full shadow-sm text-white bg-red-600 hover:bg-red-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-red-500 font-almarai"
                        >
                          قبول
                        </button>
                        <button
        ...
        <div>
          ...
          <div className="flex flex-row flex-wrap justify-center border-b border-gray-200 mb-4 sm:mb-5 mt-2">
            ...
            >
              ...
              المستلمة
              {pendingReceivedRequests.length > 0 && (
                <span className="bg-red-100 text-red-800 text-xs px-2 py-0.5 rounded-full font-almarai">
            ...
            <div className="max-h-[330px] overflow-y-auto space-y-2">
              {pendingReceivedRequests.length === 0 ? (
                <div className="text-center py-6 sm:py-8">
                  ...
                  <h3 className="mt-2 text-base sm:text-lg font-medium text-gray-900 font-almarai">لا توجد طلبات مستلمة</h3>
                  <p className="mt-1 text-xs sm:text-sm text-gray-500 font-almarai">ستظهر طلبات الصداقة التي تتلقاها هنا</p>
                </div>
              ) : (
                pendingReceivedRequests.map((request) => (
                  <div key={request.id} className="flex sm:flex-row items-center justify-between p-3 sm:p-4 bg-white border border-gray-100 rounded-lg shadow-sm hover:shadow-md transition-shadow">
                    <div className="flex items-center space-x-3 sm:space-x-4">
                      ...
                      <div>
                        <p className="text-xs sm:text-sm font-medium text-gray-900 font-almarai">{request.name_account}</p>
                        <p className="text-[10px] sm:text-xs text-gray-500 font-almarai">تم الطلب: {new Date(request.created_at).toLocaleDateString()}</p>
                      </div>
                    </div>
                    <div className="flex flex-row gap-2 mt-2 sm:mt-0">
                      <button
                        onClick={() => handleAcceptFriendRequest(request.id)}
                        className="inline-flex items-center px-2.5 py-1 border border-transparent text-xs font-medium rounded-full shadow-sm text-white bg-green-600 hover:bg-green-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-green-500 font-almarai"
                      >
                        قبول
                      </button>
                      <button
            ...
            <div className="max-h-[330px] overflow-y-auto space-y-2">
              {pendingSentRequests.length === 0 ? (
                <div className="text-center py-6 sm:py-8">
                  ...
                  <h3 className="mt-2 text-base sm:text-lg font-medium text-gray-900 font-almarai">لا توجد طلبات مرسلة</h3>
                  <p className="mt-1 text-xs sm:text-sm text-gray-500 font-almarai">ستظهر طلبات الصداقة التي ترسلها هنا</p>
                </div>
              ) : (
                pendingSentRequests.map((request) => (
                  <div key={request.id} className="flex sm:flex-row items-center justify-between p-3 sm:p-4 bg-white border border-gray-100 rounded-lg shadow-sm hover:shadow-md transition-shadow">
                    <div className="flex items-center space-x-3 sm:space-x-4">
                      ...
                      <div>
                        <p className="text-xs sm:text-sm font-medium text-gray-900 font-almarai">{request.name_account}</p>
                        <p className="text-[10px] sm:text-xs text-gray-500 font-almarai">تم الإرسال: {new Date(request.created_at).toLocaleDateString()}</p>
                      </div>
                    </div>
                    <button
                      onClick={() => handleCancelFriendRequest(request.id)}
                      className="inline-flex items-center px-2.5 py-1 border border-transparent text-xs font-medium rounded-full shadow-sm text-white bg-red-600 hover:bg-red-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-red-500 font-almarai mt-2 sm:mt-0"
                    >
                      إلغاء
                    </button>
                  </div>
