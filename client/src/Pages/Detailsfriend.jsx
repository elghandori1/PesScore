...
function Detailsfriend() {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      setFriendship(
        response.data.friendship || { status: "active", requested_by: null }
      );
      ...
      const rejectedCount = response.data.matches.filter(
        (m) =>
  ...
  const handleCancelDeletionRequest = async (matchId) => {
    ...
    try {
      ...
      setError(
        err.response?.data?.error || "Failed to cancel deletion request"
      );
    } finally {
      setLoading(false);
    }
  ...
  const handleRespondToDeletion = async (matchId, accept) => {
    ...
    try {
      ...
      setError(
        err.response?.data?.error || "Failed to respond to deletion request"
      );
    } finally {
      setLoading(false);
    }
  ...
  const filteredMatches = matches.filter((match) =>
    activeTab === "history"
      ...
      ? match.status === "pending" ||
        (match.status === "rejected" && match.created_by === currentUserId)
      : false
  );
                ...
                {filteredMatches.length === 0 ? (
                  <div className="text-center py-6 xs:py-8">
                    ...
                    <h3 className="mt-2 text-base xs:text-lg font-medium text-gray-900 font-almarai">
                      لا توجد مباريات{" "}
                      {activeTab === "history" ? "في التاريخ" : "معلقة"} تم
                      العثور عليها
                    </h3>
                              ...
                              {activeTab === "pending" ? (
                                ...
                                <>
                                  {deletionRequests[match.id] ? (
                                    deletionRequests[match.id].requested_by ===
                                    currentUserId ? (
                                      <div className="flex flex-col items-center">
                                        <button
                                          onClick={() =>
                                        ...
                                        <small className="mt-2 text-red-500 font-almarai text-[0.7rem] xs:text-[0.75rem] sm:text-[0.85rem] md:text-[0.9rem]">
                                          لقد طلبت حذف هذه النتيجة
                                        </small>
                                      </div>
                                    ) : (
                                      <div className="flex flex-col items-center">
                                        ...
                                        {/* Message for deletion request */}
                                        <small className="mt-2 text-red-500 font-almarai text-[0.7rem] xs:text-[0.75rem] sm:text-[0.85rem] md:text-[0.9rem]">
                                          صديقك يريد حذف هذه النتيجة
                                        </small>
                                      </div>
                                    )
                                  ) : (
                                    <button
                                      onClick={() =>
                                        handleRequestDeletion(match.id)
                                      }
                                      className="px-3 py-1 bg-red-500 text-white text-xs sm:text-sm rounded hover:bg-red-600 transition font-almarai"
                                    >
                                      حذف
