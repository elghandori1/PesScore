            ...
            <>
              ...
              <div className="flex justify-between mb-6">
                <div className="flex flex-col justify-center items-start">
                  <h2 className="text-lg xs:text-xl sm:text-2xl font-bold text-gray-800 break-words">
                    {friend.name_account}
                  </h2>
                  <p className="text-xs xs:text-sm text-gray-500 break-all">ID: {friend.id_account}</p>
                </div>
    
                {friendship.status === 'active' ? (
                  <Link
                    to={`/newmatch/${id}`}
                    className="inline-flex items-center justify-center px-3 sm:px-2 bg-gradient-to-r from-blue-600 to-blue-500 text-white font-medium rounded-full shadow-md hover:shadow-lg transition-all duration-200 hover:from-blue-700 hover:to-blue-600 text-xs sm:text-base"
                  >
                  ...
                  <span
                    className="inline-flex items-center justify-center px-3 sm:px-2 bg-gray-400 text-white font-medium rounded-full shadow-md text-xs sm:text-base cursor-not-allowed"
                    title="Cannot create new match while removal request is pending"
                  >
                    <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4 sm:h-5 sm:w-5 mr-1" viewBox="0 0 20 20" fill="currentColor">
                      <path fillRule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm1-11a1 1 0 10-2 0v2H7a1 1 0 100 2h2v2a1 1 0 102 0v-2h2a1 1 0 100-2h-2V7z" clipRule="evenodd" />
                    </svg>
                ...
                <div className="mb-4 p-4 bg-red-100 rounded-md">
                  <p className="text-sm text-red-800">
                    {friendship.requested_by === currentUserId
                      ? 'You have requested to remove this friend.'
                      : 'This friend has requested to end the friendship.'}
                  </p>
                </div>
              )}

              <div className="mb-1 text-xs xs:text-sm">
                Total matches: <span className='font-bold text-blue-600'>{filteredMatches ? filteredMatches.length : 0}</span>
              </div>
              ...
              <div className="flex border-b border-gray-200 mb-4">
                <button
                  ...
                  className={`flex-1 py-2 font-medium text-xs xs:text-sm sm:text-base flex items-center justify-center gap-2 ${
                    activeTab === 'history'
              ...
              <div className="max-h-[380px] overflow-y-auto space-y-2">
                {filteredMatches.length === 0 ? (
                  <div className="text-center py-6 xs:py-8">
                    ...
                    <h3 className="mt-2 text-base xs:text-lg font-medium text-gray-900">
                      No {activeTab === 'history' ? 'history' : 'pending'} matches found
                    </h3>
                    <p className="mt-1 text-xs xs:text-sm text-gray-500">
                      {activeTab === 'history'
                        ? 'Your match history with this friend will appear here'
                          ...
                          <div className="flex flex-col items-center">
                            <p className="text-xs flex flex-col items-center">
                              ...
                              <span className='font-bold'>
                                {match.winner_id === null || match.winner_id === 0 ? 'Draw' : 'Winner'}
                              </span>
                            </p>
                            {activeTab === 'history' && match.winner_id && (
                                  ...
                                  {match.status === 'rejected' ? (
                                    match.created_by === currentUserId && (
                                      <>
                                        <button
                                          ...
                                          className="px-3 py-1 bg-blue-500 text-white text-xs sm:text-sm rounded hover:bg-blue-600 transition"
                                        >
                                          Resend
                                        </button>
                                        <button
                                          onClick={() => handleCancelMatch(match.id)}
                                          className="px-3 py-1 bg-red-500 text-white text-xs sm:text-sm rounded hover:bg-red-600 transition"
                                        >
                                          Delete
                                        </button>
                                      </>
                                    ...
                                    <>
                                      <button
                                        ...
                                        className="px-3 py-1 bg-green-500 text-white text-xs sm:text-sm rounded hover:bg-green-600 transition"
                                      >
                                        Accept
                                      </button>
                                      <button
                                  ...
                                  {deletionRequests[match.id] ? (
                                    deletionRequests[match.id].requested_by === currentUserId ? (
                                      <button
                                        ...
                                        className="px-3 py-1 bg-gray-500 text-white text-xs sm:text-sm rounded hover:bg-gray-600 transition"
                                      >
                                        Cancel
                                      </button>
                                    ) : (
