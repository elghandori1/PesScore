...
function NewMatch() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await axios.post(
        "http://localhost:5000/new-matches",
        {
      ...
      <main className="flex flex-col items-center w-full pt-24 md:pt-20 px-4 pb-8">
        <section className="bg-white backdrop-blur-lg p-4 rounded-xl shadow-lg w-full max-w-md md:max-w-2xl">
          ...
          <form onSubmit={handleSubmit} className="space-y-4 md:space-y-6">
            <h2 className="text-xl md:text-2xl font-bold text-center text-blue-600">New Match</h2>
            ...
            <div>
              ...
              <div
                className="w-full px-3 py-2 border-0 rounded-lg bg-gray-200 text-blue-600"
              >
                {friend && friend.account_name || ""}
              </div>
            </div>
